                ...
                <div class="col-sm-12 col-md-4">
                    ...
                    <form class="user" enctype="multipart/form-data" action="{{ route('cohort.store')}}" method="POST">
                        @csrf
                          <div class="form-group row">
                            <div class="col-sm-12">
                                <a href="javascript:void(0)">
                                    <img id="newBoardImage" onclick="document.querySelector('#newBoardLogo').click()" class="img-profile rounded-circle d-block m-auto" style="height: 250px; width: 250px" src="{{ asset('storage/undraw_posting_photo.svg')}}">
                                </a>
                              <input type="file" name="image" class="form-control form-control-user d-none @error('image') is-invalid @enderror" id="newBoardLogo">
                              @error('image')
                          ...
                          <div class="form-group row">
                              <div class="col-sm-12">
                                <textarea class="w-100 @error('about') is-invalid @enderror" name="about" id="newBoardDescription" required>About Board</textarea>
                                @error('about')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
              ...
              @foreach ($cohorts as $cohort)
                  @if ($cohort->category == 'board')
                      <tr id="m_{{$cohort->id}}" role="row" class="odd m-category">
                          ...
                          <td>
                            <a href="javascript:void(0)" class="btn btn-success btn-circle btn-sm" data-toggle="modal" data-target="#m{{$cohort->id}}">
                                <i class="fas fa-file-alt"></i>
                            </a>
                            <a href="javascript:void(0)" class="btn btn-danger btn-circle btn-sm" data-toggle="modal" data-target="#m{{$cohort->id}}">
                                      ...
                                      <div class="row">
                                          ...
                                          <form class="user" enctype="multipart/form-data" action="{{ route('cohort.update', ['cohort' => $cohort->id])}}" method="POST">
                                              ...
                                              @method('PATCH')
                                                  ...
                                                  <div class="col-sm-12">
                                                      <a href="javascript:void(0)">
                                                          <img id="updateBoardImage_m_{{$cohort->id}}" onclick="document.querySelector('#updateBoardLogo_m_{{$cohort->id}}').click()" class="img-profile rounded-circle d-block m-auto" style="height: 150px; width: 150px" src="{{ asset('storage/' . $cohort->image) ?? asset('template/image/undraw_posting_photo.svg')}}">
                                                      </a>
                                                      <input type="file" name="image" class="form-control form-control-user d-none @error('image') is-invalid @enderror" id="updateBoardLogo_m_{{$cohort->id}}" placeholder="Logo">
                                                      @error('image')
                                                          <span class="invalid-feedback" role="alert">
                                                              <strong>{{ $message }}</strong>
                                                          </span>
                                                  ...
                                                  <input type="text" name="name" value="{{old('name') ?? $cohort->name}}" class="form-control form-control-user  @error('name') is-invalid @enderror" id="updateBoardName_m_{{$cohort->id}}" placeholder="Name" required>
                                                  @error('name')
                                                      <span class="invalid-feedback" role="alert">
                                                          <strong>{{ $message }}</strong>
                                                      </span>
                                                  ...
                                                  <div class="form-group row">
                                                      ...
                                                      <textarea name="about"  cols="60" class="form-control form-control-user @error('about') is-invalid @enderror" id="updateBoardDescription_m_{{$cohort->id}}" required>{{old('about') ?? $cohort->about}}</textarea>
                                                      @error('about')
                                                          <span class="invalid-feedback" role="alert">
                                                              <strong>{{ $message }}</strong>
                                                          </span>
