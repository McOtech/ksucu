            ...
            <div class="dataTables_length" id="dataTable_length"><label>Show <select name="dataTable_length" aria-controls="dataTable" class="custom-select custom-select-sm form-control form-control-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-12 col-md-3"><div id="dataTable_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="dataTable"></label></div></div></div><div class="row">
                <div class="col-sm-12 col-md-4">
                    <div class="text-center">
                      <h1 class="h4 text-gray-900 mb-4">Register Ministry</h1>
                    </div>
                <form class="user" enctype="multipart/form-data" action="{{ route('cohort.store')}}" method="POST">
                    @csrf
                      <div class="form-group row">
                        <div class="col-sm-12">
                            <a href="javascript:void(0)">
                                <img id="newMinistryImage" onclick="document.querySelector('#newMinistryLogo').click()" class="img-profile rounded-circle d-block m-auto" style="height: 250px; width: 250px" src="{{ asset('storage/undraw_posting_photo.svg')}}">
                            </a>
                          <input type="file" name="image" class="form-control form-control-user d-none @error('image') is-invalid @enderror" id="newMinistryLogo">
                          @error('image')
                              <span class="invalid-feedback" role="alert">
                                  <strong>{{ $message }}</strong>
                              </span>
                          @enderror
                          <input type="hidden" name="category" value="ministry">
                        </div>
                      ...
                      <div class="form-group row">
                          <div class="col-sm-12">
                            <textarea class="w-100 @error('about') is-invalid @enderror" name="about" id="newMinistryDescription" required>About Ministry</textarea>
                            @error('about')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    @if ($cohort->category == 'ministry')
                        <tr id="m_{{$cohort->id}}" role="row" class="odd m-category">
                            ...
                            @if ($cohort->membership != null)
                                @foreach ($cohort->membership as $leader)
                                    @if ($leader->post == 'chairperson' && $leader->right == 'yes')
                                        {{$chair = $leader->user->profile->fname}}
                                    @endif
                                @endforeach
                            @else
                            @endif
                            ...
                            <td class="">
                                ...
                                <span>
                                    <form action="{{ route('cohort.delete', ['cohort' => $cohort->id])}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <div class="row">
                                            ...
                                            <form class="user" enctype="multipart/form-data" action="{{ route('cohort.update', ['cohort' => $cohort->id])}}" method="POST">
                                                ...
                                                @method('PATCH')
                                                    ...
                                                    <div class="col-sm-12">
                                                        <a href="javascript:void(0)">
                                                            <img id="updateMinistryImage_m_{{$cohort->id}}" onclick="document.querySelector('#updateMinistryLogo_m_{{$cohort->id}}').click()" class="img-profile rounded-circle d-block m-auto" style="height: 150px; width: 150px" src="{{ asset('storage/' . $cohort->image) ?? asset('template/image/undraw_posting_photo.svg')}}">
                                                        </a>
                                                        <input type="file" name="image" class="form-control form-control-user d-none @error('image') is-invalid @enderror" id="updateMinistryLogo_m_{{$cohort->id}}" placeholder="Logo">
                                                        @error('image')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
                                                    ...
                                                    <input type="text" name="name" value="{{old('name') ?? $cohort->name}}" class="form-control form-control-user  @error('name') is-invalid @enderror" id="updateMinistryName_m_{{$cohort->id}}" placeholder="Name" required>
                                                    @error('name')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <textarea name="about"  cols="60" class="form-control form-control-user @error('about') is-invalid @enderror" id="updateMinistryDescription_m_{{$cohort->id}}" required>{{old('about') ?? $cohort->about}}</textarea>
                                                        @error('about')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
...
@section('nicEdit')
    <script type="text/javascript">
        ...
        $(document).ready(() => {
            document.querySelector('#newMinistryDescription').previousSibling.firstChild.addEventListener('input', function(){
                document.querySelector('#newMinistryDescription').value = this.innerHTML;
                // alert('working')
            })

            //show chosen logo
            $('#newMinistryLogo').change(function() {
                var reader = new FileReader();
                reader.onload = function(){
                    let dataUrl = reader.result
                    document.querySelector('#newMinistryImage').src = dataUrl
                }

                let file = $('#newMinistryLogo').prop('files')[0]
                reader.readAsDataURL(file)
            })
